...
/*.h only files*/
#include "diScore.h" /*defined variables only*/
...
needleScan_diScan(
   ...
   signed int *kmerArySI,     /*holds sequence kmers*/
   signed int *cntArySI,      /*holds kmer counts*/
   float minPercMatchF,     /*min % matches to be mapped*/
   float minKmerPercF,         /*min perc kmers to keep*/
   unsigned char lenKmerUC,    /*length of one kmer*/
   unsigned int minDIDelUI,   /*min del size in DI*/
   unsigned int minPadNtUI,   /*min start/end length*/
   ...
   signed char errSC = 0;       /*for detecting errors*/
   signed int numDISI = 0;      /*number of DI events*/
   float scoreF = 0;            /*score from alignment*/
   unsigned int numAnonUI = 0; /*# anonymous nt (ignore)*/
   ...
   *   - align to foward reference
   ...
   { /*If: best match forward segment*/
      ...
      if(scoreF == def_memErr_needle)
      { /*If: low score*/
         indexToSeq_alnSet(
            refAryST[*segSIPtr].forSeqST->seqStr
         );

         goto memErr_fun03_sec06; /*memory error*/
      } /*If: low score*/
   ...
   *   - align to reverse complement reference
   ...
   /*account for using integer values for scores*/
   scoreF = (float) samSTPtr->numMatchUI;
   scoreF /= seqSTPtr->lenSeqUL;

   if(scoreF < minPercMatchF)
      goto noMatch_fun03_sec06;
...
pfrag_diScan(
   ...
   fprintf(
      ...
      samSTPtr->refEndUI + 1,
      segLenSI,
      (float) scoreSL / (float) def_adjust_diScore
...
rmEndDels_diScan(
   ...
   *   - remove starting large deletions
   ...
   { /*Loop: find end*/
      ...
      if(samSTPtr->cigTypeStr[cigPosSI] == '\0')
      { /*If: cigar length off*/
         samSTPtr->lenCigUI = (unsigned int) cigPosSI;
         return; /*end of read*/
      } /*If: cigar length off*/
   ...
   { /*Loop: remove starting deletions*/
      ...
      { /*Switch: find cigar type*/
         ...
         case 'D':
            ...
            delPosSI = cigPosSI;
            ++cigPosSI;
            goto endLoop_fun06_sec04_sub01;
      } /*Switch: find cigar type*/
   } /*Loop: remove starting deletions*/

    endLoop_fun06_sec04_sub01:;
